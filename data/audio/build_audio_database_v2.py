...
def generate_path_list(sample_range=SAMPLE_RANGE,repo_path=WAV_REPO_PATH,frame_path=FRAME_LOG_PATH):
    ...
    for i in range(len(frames)):
        frame = frames[i].replace('\n','').replace('frame_','') #frame_0-1 -> 0-1
        frame_set.add(frame) #trim_audio_train__.wavを対応されるためには0-1の形を残さないといけない。
    index_counter=0
    for i in range(sample_range[0],sample_range[1]):
        print('\rchecking...%s'%i,end='') #end=''で改行、空白をなくす
...
# data generate function single-00001.npy indexは動画インデックスとは関係ないものを出力
...
# 入力するためのmix spectrogramを生成するために、音声の合成、合成音からのspectrogram生成
...
def single_crm(idx_str_list,mix_path,database_repo):
    ...
    for idx in idx_str_list:
        ...
        #mix spectrogramからsingle spectrogramを抽出するためのマスク生成
