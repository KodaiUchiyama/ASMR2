...
def cut(loc,name,segment_name,start_time):
    # trim the audio/video by sox
    # loc         | the location of the file
    # name        | the name of file to trim | ex,audio_train1, audio_train2
    # start_time  | the start time of the audio segment
    # end_time(deleted)    | the end time of the audio segment
    length = 3
    command = 'cd %s;' % loc
    ...
    command += 'sox %s.wav %s.wav trim %s %s;' % (name,segment_name,start_time,length)
    os.system(command)
