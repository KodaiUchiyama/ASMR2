...
def VO_model():
    ...
    conv7 = UpSampling2DBilinear((298, 256))(conv7)
    conv7 = Reshape((298, 256))(conv7)
    print('7:', conv7.shape)
    '''
    # --------------------------- VS_model start ---------------------------
    VS_model = Sequential()
    ...
    lstm = Bidirectional(LSTM(400,input_shape=(298,256),return_sequences=True),merge_mode='sum')(AVfusion)
    ...
    complex_mask = Dense(257 * 2 , name="complex_mask", kernel_initializer=glorot_uniform(seed=87))(fc3)
    print('complex_mask:', complex_mask.shape)

    complex_mask_out = Reshape((298, 257, 2))(complex_mask)
    print('complex_mask_out:', complex_mask_out.shape)

    # --------------------------- VO end ---------------------------
    VO_model = Model(inputs=model_input, outputs=complex_mask_out)
